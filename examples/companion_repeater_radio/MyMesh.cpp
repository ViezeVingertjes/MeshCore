...
bool MyMesh::allowPacketForward(const mesh::Packet* packet) {
  // Forward flood packets up to a reasonable hop/path limit to avoid endless propagation.
  // Direct packets are forwarded only when this node is the next hop (handled in Mesh.cpp)
  // Keep it permissive so the companion participates in repeating.
  const uint8_t MAX_FLOOD_PATH = 64; // same default as repeater example
  if (packet->isRouteFlood()) {
    if (packet->path_len >= MAX_FLOOD_PATH) return false;
    return true;
  }
  return true;
}