...
void Dispatcher::loop() {
  ...
  if (isPowerSavingEnabled()) {
    ...
    if (radio_listening && !has_outbound) {
      ...
      if (board) {
        if (idle_ms > getDeepSleepIdleMs()) {
          board->enterDeepSleep(getDeepSleepDurationSecs());
          // Note: deep sleep will cause a device reset, code below won't execute
        } else if (idle_ms > getLightSleepIdleMs()) {
          board->enterLightSleep(getLightSleepSliceMs());
          last_activity_ms = _ms->getMillis(); // Update activity time after waking up
        }
      }
    }
  }
